...
Dijstra est propose avec un TAS de FIBONNACI
Dijstra sera beaucoup plus efficasse avec des matrices de ce style
 ...
 Mais les exemple propos� sur moodle et ceux du Project Euler sont beaucoup moins disparatre
 dans les valeurs formant le meilleur chemin .
 
 Et donc l'apporche dynamique en place est plus efficasse dans le cas simple a cause de la 
 complexite en espace de l'implementation de disjtra bien que disjtra economise en general 
 de visiter environ 20% des sommets de la matrice ( dans les grands exemple).
...
public class DijstraWithFibonnaci {

	
	public static void main(String[] args) throws Exception {
		
		/*
		Som [][] matrix = new Som[1000][1000];
		
		Random random = new Random();
		
		for(int i=0;i<matrix.length;i++){
			for(int j=0;j<matrix.length;j++){
				//int alea = random.nextInt(20) + 1;
				matrix[i][j]=new Som(random.nextInt(100)+1, i, j);
			}
		}
		
		System.out.println("CREATION MATRIX ALEA 1000 * 1000");
		*/
		
		Som[][] matrix = getMatrix(args[0]);
		
		long startTime = System.currentTimeMillis();
		
		
		String dij3=Dijkstra(matrix,true);

		String run=run(matrix);
		
		System.out.println(run);
		System.out.print(dij3);
	...
	static class Som{
		...
		public boolean notAcceded=true;
		FibonacciHeap.Entry<Som> entrie;
		
		public Som prev;
		public String prevString;
		
		public Som(int value, int i , int j) {
			this.value = value;
			this.i=i;
			this.j=j;
		}
	...
	public static String run(Som [][] grid) {
		// Dynamic programming
			...
			for (int j = grid[i].length - 1; j >= 0; j--) {
				...
				if (i + 1 < grid.length && j + 1 < grid[i].length){
					...
					if(grid[i + 1][j].value<grid[i][j + 1].value){
						temp=grid[i + 1][j].value;
						
					}else{
						temp=grid[i][j+1].value;
					}
				}
				...
				//System.out.println("I = "+i+"  | J="+j+"  valeur actuel ="+temp+"  "+tempS);
				grid[i][j].value+= temp;
		...
		int j=0;
		String path="";
		int sizeMax=grid.length;
		while(!(i==sizeMax-1 && j==sizeMax-1)){
			if(i+1==sizeMax){
				j++;
				path+="D";
			}
			...
			else if(grid[i+1][j].value<grid[i][j+1].value){
				path+="B";
				i++;
			}else{
				j++;
	...
	public static String Dijkstra(Som[][] grid,boolean WithUP){
		
		FibonacciHeap<Som> file =new FibonacciHeap<Som>();
		
		grid[0][0].entrie=file.enqueue(grid[0][0],grid[0][0].value );
		
		Integer valPath;
		
		FibonacciHeap.Entry<Som> curr,dest;
		...
		while (!file.isEmpty()) {
			curr= file.dequeueMin();
			curr.getValue().notAcceded=false;
			
			//result.add(curr.getValue());
			//nb++;
			//System.out.println("AJOUT : "+curr.getValue().i +" j = "+curr.getValue().j+" VAL:"+curr.getValue().value);
			
			if(curr.getValue().i==grid.length-1 && curr.getValue().j==grid.length-1){
				
				//System.out.println("nb passage : "+nb);
				
				Som tmp=curr.getValue().prev;
				String chemin="";
				while(tmp!=null){
					
					if(tmp.prevString!=null){
						if(chemin==""){
							chemin=tmp.prevString;	
						}
						chemin=tmp.prevString+","+chemin;
					}
					tmp=tmp.prev;
					
				}
				
				return (int)curr.getPriority()+"\n"+chemin;
			}
			
			
			tmpI=curr.getValue().i;
			tmpJ=curr.getValue().j;
			
			if(tmpI+1<grid.length){
				Som bas=grid[tmpI+1][tmpJ];
				
				//System.out.println("dans BAS : "+bas.value);
				//if (!result.contains(bas)){
					
				if (bas.notAcceded){
					valPath=(int) (curr.getPriority()+bas.value);

					if(bas.entrie==null){
						bas.entrie=file.enqueue(bas, valPath);
						bas.prev=curr.getValue();
	    				bas.prevString="B";
					}else{
						dest = bas.entrie;
						
						if (valPath < dest.getPriority()){
		                    file.decreaseKey(dest, valPath);
		                    bas.prev=curr.getValue();
		    				bas.prevString="B";
						}
					}
			...
			if(WithUP){
				if(tmpI>0){
					Som haut=grid[tmpI-1][tmpJ];
					
					//System.out.println("dans HAUT : "+bas.value);
					//if (!result.contains(haut)){
					if (haut.notAcceded){
						valPath=(int) (curr.getPriority()+haut.value);

						boolean apply=false;
						if(haut.entrie==null){
							haut.entrie=file.enqueue(haut, valPath);
							apply=true;
						}else{
							dest = haut.entrie;
							if (valPath < dest.getPriority()){
			...
			if(tmpJ+1<grid.length){
				
				Som droite=grid[tmpI][tmpJ+1];
				
				//System.out.println("dans  DROITE: "+droite.value);
				//if (!result.contains(droite)){
				if (droite.notAcceded){
					valPath=(int) (curr.getPriority()+droite.value);
					
					if(droite.entrie==null){
						droite.entrie=file.enqueue(droite, valPath);
						droite.prev=curr.getValue();
	    				droite.prevString="D";
					}else{
						dest = droite.entrie;
						if (valPath < dest.getPriority()){
	 ...
	 */
	public static Som[][] getMatrix(String path) throws Exception {

		File file = new File(path);
		BufferedReader br = new BufferedReader(new FileReader(file));

		
		String[] tab = br.readLine().replaceAll("^( )+","").split(",");
		int size = tab.length;
		Som[][] matrix = new Som[size][size];
		int position = 0;
		
		for (int i = 0; i < tab.length; i++) {
			matrix[position][i] =  new Som(Integer.parseInt(tab[i]),position,i);
		}
		position++;
	 ...
	 */
	public static void showMatrix(Som [][] matrix){
		System.out.println("");
		for(int i=0; i<matrix.length;i++){
			for(int j =0; j<matrix.length;j++){
